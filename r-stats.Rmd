...
# What if you turn it into a factor variable?
factor(nha$SmokingStatus)

# Now let's relevel it and see what that looks like. 
...
```{r, results="hide"}
#Look at Race: it's a character vector
nha$Race

# What if we coerce it into a factor?
factor(nha$Race)

# Default levels are alphabetical: Asian Black Hispanic Mexican Other White
# Let's relevel that where the group with the highest rate of insurance is "baseline"
relevel(factor(nha$Race), ref="White")

# If we're happy with that result, permanently change it
nha$Race <- relevel(factor(nha$Race), ref="White")
```

Now, let's fit a logistic regression model assessing how the odds of being insured change with different levels of race. Before we do that we need to turn "Insured" into a factor variable also.

```{r, results="hide"}
# Take a look
nha$Insured

# Factor it, take a look
factor(nha$Insured)

# Make the factor coercion stick
...
2. Model the same association in a logistic regression framework to assess the risk of diabetes using physical activity as a predictor.
    - First, make Diabetes a factor variable (`nha$Diabetes <- factor(nha$Diabetes)`).
