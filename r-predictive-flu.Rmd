...
Starting with the [original dataset](data/h7n9.csv) we're going to do some exploratory data analysis, and we'll extract and recode _features_ from the raw data, including age, days between onset and hospitalization or outcome, gender, and the patient's province. We will eventually want to traing a model to predict the patient's outcome. We'll do this by training and testing different models on the data where the outcome is known, and then apply these models to cases where we don't know the outcome. There are lots of missing data points in other variables, so we'll _impute_ those variables to maximize our use of the data. 
...
- Hospitalization: let's create a binary classifier where  =not hospitalized, 1=hospitalized.
- Early onset: let's create another binary 0/1 indicating whether someone had an early outcome (earlier than the median outcome date observed).
...
Similarly, let's get an indicator whether someone was hospitalized or not. If hospitalization is missing, this would return TRUE.
...
_Optional_: Notice how the new variables are provinceJiangsu, provinceOther, provinceShanghai, provinceZhejiang. If we want to strip off the "province" we can do that. There's a built-in command called `gsub` that can help here.
...
Each of the models may have one or more _tuning parameters_ -- some value or option you can set to tweak how the algorithm develops. In k-nearest neighbors, we can try different values of _k_. With random forest, we can set the _mtry_ option -- the algorithm will select _mtry_ number of predictors to attempt a split for classification. Caret attempts to do this using a procedure like this:
...
Take a look at what that tells us. It tells us it's fitting a Random Forest model using 77 samples, predicting a categorical outcome class (Death or Recover) based on 10 predictors. It's not doing any pre-processing like centering or scaling, and it's doing bootstrap resampling of 77 samples with replacement, repeated 25 times each. Random Forest has a single tuning parameter, mtry -- the algorithm will select _mtry_ number of predictors to attempt a split for classification when building a classification tree. The caret package does 25 bootstrap resamples for different values of mtry (you can also control this too if you want), and computes [accuracy](https://en.wikipedia.org/wiki/Sensitivity_and_specificity#Confusion_matrix) and [kappa](https://en.wikipedia.org/wiki/Cohen%27s_kappa) measures of performance on the held-out samples. 
...
[k-nearest neighbor](https://en.wikipedia.org/wiki/K-nearest_neighbors_algorithm) attempts to assign samples to their closest labeled neighbors in high-dimensional space. You'll need the kknn package for this.
