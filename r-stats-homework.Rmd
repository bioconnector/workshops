...
knitr::opts_chunk$set(echo = FALSE, message = FALSE, warning = TRUE, cache=TRUE)
```
...
We'll be using the data behind this paper to answer the questions that follow. Use the following to read and store the data directly from the web. Feel free to load dplyr+readr and use the `read_csv()` function instead of base R's `read.csv()`.
...
stress <- read_csv("http://biostat.mc.vanderbilt.edu/wiki/pub/Main/DataSets/stressEcho.csv")
```
...
dmd <- read_csv("http://biostat.mc.vanderbilt.edu/wiki/pub/Main/DataSets/dmd.csv")
```
