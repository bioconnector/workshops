...
We need to do a few things first before we fit a logistic regression model. When we load in data with `read_csv()` from the **readr** package, by default it does not convert strings to **factors**. Factors are like a special kind of string or character variable, where the values take on a limited number of _discrete_, distinct values. You can see what each type of variable is by running `glimpse(nha)`. You can specifically see what kind of object a variable is by calling `class(nha$Insured)`, for example. 

We need to convert our outcome variable (Insured) and the Race variable, which we'll use for modeling, into factor variables. While we're at it, let's _relevel_ the Race variable so that "White" is the baseline. We saw above that people who identify as "White" have the highest rates of being insured. When we run the logistic regression, we'll get a separate coefficient (effect) for each level of the factor variable(s) in the model, telling you the increased odds that that level has, _as compared to the baseline group_.
